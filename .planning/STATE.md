...
- **Escaneo en bloque de jobhunt: LISTO (15 jul).** `aptus jobs [--pack] [--limit]`: evalúa de una vez las ofertas de `jobs.db` (402 reales, 1,8 s). Sin motor nuevo: cada oferta recorre el mismo camino que `aptus jd` y con la MISMA evidencia (tu última medición), que es lo que las hace comparables. Solo lectura; no importa el scoring propio de jobhunt (`score_cv_match`) — es su criterio y colaría el "% de encaje" por la puerta de atrás.
  ...
  - **PROBLEMA ABIERTO (visto con datos reales, pendiente de decisión):** ordenar por escalones descendente pone arriba las ofertas para las que estás MÁS sobrecualificado (top 5 = puestos junior, +2), y hunde la Senior AI Engineer que sería la interesante (-1). La lista responde "¿para qué estoy más pasado de nivel?" en vez de "¿qué me conviene mirar?". El agrupado por veredicto (opción A descartada) no tenía este defecto.
  ...
  - **PENDIENTE conocido:** `market_keywords` de `ai-product-system-design` incluye palabras de oficina genéricas ("product", "producto", "stakeholder", "arquitectura") que aparecen en CUALQUIER oferta. Valían para su uso original (ponderar demanda sobre todo el corpus, donde el ruido se promedia), pero envenenan la clasificación de UNA oferta: Caterpillar (pruebas de vehículos) dice "product" 14 veces y sigue colándose en el top. Reutilizar `market_keywords` para el JD fue decisión de diseño mía y tiene esta fuga. Arreglo probable: que el pack distinga keywords fuertes (específicas) de débiles (genéricas, solo apoyan, nunca hacen núcleo).
