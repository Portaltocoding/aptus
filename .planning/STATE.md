...
- **Banco mucho mayor: EN CURSO.** Objetivo ~250 (50/dim). Actual: **219/250**. Por dimensión: llm-rag-evals **55 ✓**, ml-clasico **50 ✓**, fullstack-next-nest-ts **50 ✓**, ai-product-system-design **50 ✓** (todas con ~5 experto), comportamental-star **14** (falta +36 con experto). Última dimensión pendiente: comportamental-star. Escrito a mano, verificando correctitud; ids `<dim>-NNN`.
- **Generación de packs por input/fuentes (petición de Carlos, 14 jul):** quiere una lógica "le doy inputs o señalo fuentes → me arma el pack". Enfoque acordado a diseñar: (A) asistido por Claude — señala fuentes/tema → Claude lee/investiga, cura questions+readiness, verifica, escribe packs/<tema>/ (funciona hoy vía chat, se puede formalizar con un scaffold `aptus new-pack` + recipe); (B) generador in-tool con LLM + verificación obligatoria. Pendiente de construir.
