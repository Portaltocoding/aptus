...
- **Escaneo en bloque de jobhunt: LISTO (15 jul).** `aptus jobs [--pack] [--limit]`: evalúa de una vez las ofertas de `jobs.db` (402 reales, 1,8 s). Sin motor nuevo: cada oferta recorre el mismo camino que `aptus jd` y con la MISMA evidencia (tu última medición), que es lo que las hace comparables. Solo lectura; no importa el scoring propio de jobhunt (`score_cv_match`) — es su criterio y colaría el "% de encaje" por la puerta de atrás.
  - **Orden por escalones (decisión de Carlos, con la pega puesta encima de la mesa).** `levelDelta` = niveles que te sobran/faltan; cantidad real y auditable derivada del veredicto existente, no un score nuevo. Cada fila enseña su veredicto entero; jamás un porcentaje. Las tres cestas (ranked / sin nivel / no evaluables) suman siempre el total: nada se cae en silencio.
  - **Orden por escalones: PROBADO Y DESCARTADO con datos reales (15 jul).** Ponía arriba las ofertas para las que estás MÁS sobrecualificado (top 5 = puestos junior, +2) y hundía la Senior AI Engineer interesante (-1): respondía "¿para qué estoy pasado de nivel?" en vez de "¿qué me conviene mirar?". Sustituido por **cubos por veredicto** (llegas / te falta 1 / te faltan 2+), alfabético dentro de cada cubo — cualquier otro criterio volvería a insinuar un ranking. En "te falta 1" se muestra el peor punto (solo si de verdad está por debajo del umbral de gap: llamar "peor punto" a un 88% es ruido disfrazado de consejo).
  - **Bugs reales que solo aparecieron con las 402 ofertas de verdad (las fixtures de juguete los tapaban):**
    1. **Keywords solapadas contaban doble.** "producto" dispara `product` Y `producto`; "escalabilidad" dispara `escalab` Y `scalab`. Una palabra valía por dos, y `ai-product-system-design` (la dimensión con más pares ES/EN) se inflaba y se llevaba el núcleo de cualquier oferta. Arreglado: `countMentions` fusiona tramos solapados → una mención es una mención.
    2. **Sin suelo absoluto, la share mentía.** Airbus: 3 menciones sueltas de "product" = 100% de share = núcleo → una oferta de fabricación se evaluaba como puesto de producto. Arreglado con `MIN_CORE_HITS`=4.
    3. **"Mid-Market" se leía como seniority mid** (Account Executive de ventas). Arreglado limpiando falsos amigos (`LEVEL_NOISE`) antes de leer el nivel.
  - **Keywords DÉBILES (arreglo de la fuga de reutilizar `market_keywords`).** Esa lista se creó para ponderar demanda sobre TODO el corpus, donde el ruido se promedia; al reutilizarla para clasificar UNA oferta, el ruido decidía: Caterpillar (pruebas de vehículos) decía "product" 14 veces y salía como puesto de producto. Ahora `readiness.yaml` declara `weak_keywords` (opcional, retrocompatible): palabras de oficina que cuentan como mención pero NO pueden hacer núcleo a una dimensión por sí solas. Para ser núcleo hay que pasar cuatro filtros: peso relativo, evidencia absoluta, no ser solo un "valorable", y no sostenerse solo sobre palabras genéricas.
  - **Más falsos amigos del seniority, todos vistos en ofertas reales:** "without a big data science **staff**" (= plantilla; el lookahead conserva los "Staff Engineer" de verdad), "**Reporting to** an ACS AI Team Lead" (el nivel de tu jefe no es el tuyo — si acaso es lo contrario), y "Intern"/"prácticas" añadidos como junior. Un becario salía como Staff-ready.
  - **Resultado con las 402 reales:** 300 evaluables, 102 no evaluables (eran 13 antes de los filtros). Cubos: 88 llegas / 23 a un escalón / 63 más lejos. Fuera del top: Caterpillar, Airbus y un Account Executive de ventas.
