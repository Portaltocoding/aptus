...
- **Generación de packs por input/fuentes (petición de Carlos, 14 jul):** quiere una lógica "le doy inputs o señalo fuentes → me arma el pack". Enfoque A (asistido por Claude): señala fuentes/tema → Claude lee/investiga, cura questions+readiness, verifica, escribe packs/<tema>/. **PASO 1 HECHO:** comando `aptus new-pack <tema>` que crea el esqueleto aislado (pack.yaml + questions/ejemplo válido + sources/). **PENDIENTE del flujo:** (i) la "receta" repetible de currar el pack desde `sources/` (skill/proceso), (ii) pasada de verificación de correctitud, (iii) opcional generador in-tool con LLM. Enfoque B (LLM in-tool) diferido.
